...
export function ClimbPreviewCard({climbName, climbSetter,climbDesc, climbDiff, publicKey, ...props}){
    ...
    return (<div className="climb-preview-card">
        <p className={classes.difficulty} tooltip={'The difficulty of the climb'}>{climbDiff}</p> 
        <div className={classes["inner-card"]}>
            <h1>{climbName}</h1>
            <h2>{climbSetter}</h2>
            <p>{climbDesc}</p>
            <button className="action-button widest transition-action">Buy Climb</button>
        </div>
    </div>)
}
